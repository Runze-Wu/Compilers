...
/* High-level Definitions */
Program: ExtDefList {
        $$=nonterminal_node("Program",@$.first_line);
        root=$$;
        set_parent_brother($$,1,$1);
    }
;
ExtDefList: ExtDef ExtDefList {
        $$=nonterminal_node("ExtDefList",@$.first_line);
...
ExtDef: Specifier ExtDecList SEMI {
        $$=nonterminal_node("ExtDef",@$.first_line);
...
;
ExtDecList: VarDec {
        $$=nonterminal_node("ExtDecList",@$.first_line);
        set_parent_brother($$,1,$1);
    }
...
/* Specifiers */
Specifier: TYPE {
        $$=nonterminal_node("Specifier",@$.first_line);
        set_parent_brother($$,1,$1);
    }
    | StructSpecifier {
        $$=nonterminal_node("Specifier",@$.first_line);
...
StructSpecifier: STRUCT OptTag LC DefList RC {
        $$=nonterminal_node("StructSpecifier",@$.first_line);
...
;
OptTag: ID {
        $$=nonterminal_node("OptTag",@$.first_line);
        set_parent_brother($$,1,$1);
    }
    | /* empty */ {
        $$=NULL;
...
;
Tag: ID {
        $$=nonterminal_node("Tag",@$.first_line);
        set_parent_brother($$,1,$1);
    }
;
/* Declarators */
VarDec: ID {
        $$=nonterminal_node("VarDec",@$.first_line);
        set_parent_brother($$,1,$1);
    }
    | VarDec LB INT RB {
        $$=nonterminal_node("VarDec",@$.first_line);
...
FunDec: ID LP VarList RP {
        $$=nonterminal_node("FunDec",@$.first_line);
        set_parent_brother($$,4,$1,$2,$3,$4);
    }
    | ID LP RP {
        $$=nonterminal_node("FunDec",@$.first_line);
...
VarList: ParamDec COMMA VarList {
        $$=nonterminal_node("VarList",@$.first_line);
        set_parent_brother($$,3,$1,$2,$3);
    }
    | ParamDec {
        $$=nonterminal_node("VarList",@$.first_line);
...
ParamDec: Specifier VarDec {
        $$=nonterminal_node("ParamDec",@$.first_line);
        set_parent_brother($$,2,$1,$2);
    }
;
/* Statements */
CompSt: LC DefList StmtList RC {
        $$=nonterminal_node("CompSt",@$.first_line);
        set_parent_brother($$,4,$1,$2,$3,$4);
    }
;
StmtList: Stmt StmtList {
        $$=nonterminal_node("StmtList",@$.first_line);
        set_parent_brother($$,2,$1,$2);
    }
    | /* empty */ {
        $$=NULL;
...
;
Stmt: Exp SEMI {
        $$=nonterminal_node("Stmt",@$.first_line);
        set_parent_brother($$,2,$1,$2);
    }
    | CompSt {
        $$=nonterminal_node("Stmt",@$.first_line);
    ...
    | IF LP error RP Stmt  %prec LOWER_THAN_ELSE {my_yyerror();}
    | IF LP error RP Stmt ELSE Stmt {my_yyerror();}
    | WHILE LP error RP Stmt {my_yyerror();yyerrok;}
;
/* Local Definitions */
DefList: Def DefList {
        $$=nonterminal_node("DefList",@$.first_line);
        set_parent_brother($$,2,$1,$2);
    }
    | /* empty */ {$$=NULL;}
;
Def: Specifier DecList SEMI {
        $$=nonterminal_node("Def",@$.first_line);
        set_parent_brother($$,3,$1,$2,$3);
    }
...
;
DecList: Dec {
        $$=nonterminal_node("DecList",@$.first_line);
        set_parent_brother($$,1,$1);
    }
    | Dec COMMA DecList {
        $$=nonterminal_node("DecList",@$.first_line);
...
;
Dec: VarDec {
        $$=nonterminal_node("Dec",@$.first_line);
        set_parent_brother($$,1,$1);
    }
    | VarDec ASSIGNOP Exp {
        $$=nonterminal_node("Dec",@$.first_line);
...
Exp: Exp ASSIGNOP Exp {
        $$=nonterminal_node("Exp",@$.first_line);
        set_parent_brother($$,3,$1,$2,$3);
    }
    | Exp AND Exp {
        $$=nonterminal_node("Exp",@$.first_line);
...
;
Args: Exp COMMA Args {
        $$=nonterminal_node("Args",@$.first_line);
        set_parent_brother($$,3,$1,$2,$3);
    }
    | Exp {
        $$=nonterminal_node("Args",@$.first_line);
