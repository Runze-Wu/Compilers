...
void bb_tag_ir_list(InterCodeList ir_list_head, InterCodeList* labels);          // 给ir_list标记BB开始
void bb_tag_ir(InterCode ir);                                                    // 给ir标记为基本块开始
bool is_bb_start(InterCode ir);                                                  // 判断是否是BB开始
void construct_bb_list(BasicBlockList bb_list_head, InterCodeList ir_list_head, InterCodeList* labels);  // 创建BB链表
